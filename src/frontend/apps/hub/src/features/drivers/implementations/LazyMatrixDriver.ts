 ...
 * thin proxy. It still advertises composition support immediately so the UI can
 * render the same controls once the Matrix connection has completed.
 */
export class LazyMatrixDriver extends BaseDriver {
  ...
  async toggleChatThreadReaction(
    ...
    return this.withTarget((driver) =>
      driver.toggleChatThreadReaction(params),
    );
  }
